...
//?[] Building Line 2 Runs From db
//?[] Updates and Deletes Line 2 Runs From db
async function l2AppGetList(e) {
  // ************** USING LOCAL STORAGE **************************
  const jsonDb = JSON.parse(localStorage.getItem("jsonDb"));
  // ************************************************************
  try {
    // represents line2Runs table in Compounding database
    ...
    const jsonDbLine2Runs = jsonDb.line2Runs;

    const sortL2Runs = jsonDbLine2Runs.sort((a, b) =>
    ...
    const l2RunForms = document.querySelector(".l2-run-forms");
    removeAllChildNodes(l2RunForms);

    const l2SortedRuns = sortL2Runs.map((run) => {
      // ul
      const l2RunForms = document.querySelector(".l2-run-forms");
      ...
      l2RunForms.appendChild(listItem);
      ...
      // updateBtn.addEventListener("click", updateRunItem);
      async function updateRunItem(e) {
        ...
        try {
          ...
          //   `${httpAddress}:${httpPort}/line2-runs/${runSequenceInput.value}`,
          //   {
          //     method: "PUT",
